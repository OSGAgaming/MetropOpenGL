...
void main()
{
    ...
    float fov = 90.0;
    vec3 cam_o = vec3(0.0, 0.0, -tan(radians(fov) / 2.0)); // added radians() for FOV
    ...
    {
        vec3 intersection = ray_o + ray_d * (-b - sqrt(intersectionState)); // fix the intersection calculation
